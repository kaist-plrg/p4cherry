...
let type_name (typ: Il.typ): string = match S.it typ with
...
let fetch_parser_inst_name (inst_decl: Il.decl) : string = 
  ...
  let parser_arg = match parser_arg_opt with 
    | None -> raise (V1Model "Expected a parser component in V1Switch instantiation")
    | Some x -> x
  in
...
let fetch_control_inst_names (inst_decl: Il.decl) : string list = 
  ...
  in
  let () = if (List.length filtered_args <> 5) then raise (V1Model "Expected 5 control components in V1Switch instantiation") in
  List.map (fun arg -> 
    match S.it arg with 
    | L.ExprA {it; _} -> (
        match it with 
