...
let empty file includes defines = { file; includes; defines }
...
let undefine env m =
  { env with defines = List.Assoc.remove ~equal:String.equal env.defines m }
...
let rec eval_test (env : env) (test : test) : Int64.t =
  match test with
  | Int n -> n
  | Defined m -> if is_defined env m then Int64.zero else Int64.one
  | Ident _ -> Int64.zero
  | BinOp (test1, bop, test2) ->
      eval_binop bop (eval_test env test1) (eval_test env test2)
  | UnOp (uop, test1) -> eval_uop uop (eval_test env test1)
...
module Make (F : F) = struct
  ...
  let rec eval (env : env) (buf : Buffer.t) (term : term) : env =
    ...
    | Include (line, _, file) ->
        ...
        let path =
          match resolve includes file with
  ...
  and resolve includes (filename : string) : string option =
    ...
    | h :: t ->
        ...
        if F.exists path then Some path else resolve t filename

  and preprocess (env : env) (filename : string) (contents : string) :
      string * env =
    ...
    let env =
      List.fold_left ~init:env ~f:(fun env term -> eval env buf term) terms
    in
