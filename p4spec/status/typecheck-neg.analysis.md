...
Not enough arguments, relates to overhead resolution rules.
- function_e, issue2413, not_bound: fails to find a function because not enough arguments were provided.
- issue803-1: fails to find a constructor because not enough arguments were provided. 
- interface_e: fails to find a constructor because no type argument was provided.
...
Recursion
- functors2_e, self-call: controls are not recursive.
- issue3197-e: functions are not recursive.

Other
- call1_e, functors3_e: structs do not have a constructor.
- interface1_e: method does not exist.
...
(CF) -- if V? matches ()
```
- incorrect-label: wrong switch label name
...
- spec-issue563: control cannot be used as parameter type
- issue345: controls cannot be generic. The type parameter is interpreted as a free identifier.
...
- issue2290: Header types cannot be nested inside header types.
...
(CF) -- omitted ...
```
- issue2354-1, issue2354: actions cannot take parameter of `int` type
- param: actions cannot take parameter of `extern` type
...
(CF) -- omitted ...
```
Functions cannot take parameter of type `control`
...
(P4-SpecTec behavior: handled as parser error, so no premise is matched)
...
##### implicit.p4
##### signed.p4
- implicit: `int<32>` to `bit<32>`.
- signed: `int<8>` to `bit<8>`.
...
##### range_e.p4
##### issue3221.p4
- issue1006-1: `bit<16>` to `bit<8>`.
- issue1986-1: `bit<0>` to `bit<1>`.
- issue1986: `bit<0>` to `bit<2>`.
- range_e: `bit<3>` to `bit<2>`.
- issue3221: `bit<2>` to `bit<1>`.
...
##### serEnumImplCast.p4
##### issue3623-3.p4
- issue2220: `bit<8>` to a serializable enum with underlying type `bit<8>`.
- issue3623-2: `bit<4>` to a serializable enum with underlying type `bit<4>`.
- issue3534, serEnumImplCast: `bit<2>` to a serializable enum with underlying type `bit<2>`.
- issue3623-3: `int` to a serializable enum with underlying type `bit<4>`.
...
```
- issue3363: Externs cannot be compared.
(p4spec behavior: Currently passes typechecker. The above is where the typechecker is supposed to fail.)
- issue3589: Tables cannot be compared.
Involves SpecTec function arguments.
...
(CF) -- if ~$nestable_sett(typ)
```
(P4-SpecTec behavior: type restrictions are handled using nestable_sett)
`value_set` should only take types of `FIntT` and `FBitT`. No premise directly handles this, but the CF sufficiently covers counterexamples.
...
(P4-SpecTec behavior: passes typechecker. Seems there should be a premise for `id` in `Decl_ok/instd-none`.)
- issue4142: Tries to use `bit<32>` as type for `main`.
(P4-SpecTec behavior: handled as transform error)


### (4) Calls and Instantiations
...
(P4SpecTec behavior: handled as parser(transform) error, so no premise is matched.)
...
##### p_e.p4
##### shift_e.p4
(P4-SpecTec behavior: handled as parser error)
