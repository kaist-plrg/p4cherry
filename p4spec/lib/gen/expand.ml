...
let rec expand_typ depth (tdenv : TDEnv.t) (typ : typ) : value option =
  if depth <= 0 then None else expand_typ' depth tdenv typ.it
...
and expand_typ' depth tdenv typ' : value option =
  ...
  | IterT (typ_inner, List) ->
      if depth = 0 then ListV [] |> Option.some
        ...
        let* values_inner =
          List.init l (fun _ -> expand_typ depth tdenv typ_inner) |> bind_list
        in
...
let expand_init seed spec : TDEnv.t = 
  Random.init seed;
