...
type graph = Dep.Graph.t ref

let fresh () = Dep.Graph.fresh ()
let add_node = Dep.Graph.add_node ~taint:true
...
let in_pair (do_in_a : graph -> 'a -> value) (do_in_b : graph -> 'b -> value)
  ...
  let value = TupleV [ va; vb ] $$$ fresh () in
  add_node graph value;
  value
...
let in_bool (graph : graph) (boolean : bool) : value =
  let value = BoolV boolean $$$ fresh () in
  add_node graph value;
  value
...
let in_num (graph : graph) (num : P4.num) : value =
  ...
  | i, Some (width, signed) ->
      ...
      in
      let vwidth =
        let vwidth = NumV (`Nat width) $$$ fresh () in
        add_node graph vwidth;
        vwidth
      in
      let vint =
        let vint = NumV (`Int i) $$$ fresh () in
        add_node graph vint;
        vint
      in
      let value = CaseV (mixop, [ vwidth; vint ]) $$$ fresh () in
      add_node graph value;
      value
  | i, None ->
      ...
      let vint =
        ...
        vint
      in
      let value = CaseV (mixop, [ vint ]) $$$ fresh () in
      add_node graph value;
      value
...
let in_text (graph : graph) (text : P4.text) : value =
  let value = TextV text.it $$$ fresh () in
  add_node graph value;
  value
...
let in_id (graph : graph) (id : P4.id) : value =
  let value = TextV id.it $$$ fresh () in
  add_node graph value;
  value
...
let in_var (graph : graph) (var : P4.var) : value =
  ...
  | Top id ->
      ...
      let value = CaseV (mixop, [ vid ]) $$$ fresh () in
      add_node graph value;
      value
  | Current id ->
...
let rec in_member (graph : graph) (member : P4.member) : value =
  let value = TextV member.it $$$ fresh () in
  add_node graph value;
  value
...
let in_match_kind (graph : graph) (match_kind : P4.match_kind) : value =
  let value = TextV match_kind.it $$$ fresh () in
  add_node graph value;
  value
...
let in_state_label (graph : graph) (state_label : P4.state_label) : value =
  let value = TextV state_label.it $$$ fresh () in
  add_node graph value;
  value
...
let in_unop (graph : graph) (unop : P4.unop) : value =
  ...
  in
  let value = CaseV (mixop, []) $$$ fresh () in
  add_node graph value;
  value
...
let rec in_typ (graph : graph) (typ : P4.typ) : value =
  ...
  | FIntT expr ->
      ...
      let value = CaseV (mixop, [ vexpr ]) $$$ fresh () in
      add_node graph value;
      value
  | FBitT expr ->
  ...
  | StackT (typ, expr) ->
      ...
      let vexpr = in_expr graph expr in
      let value = CaseV (mixop, [ vtyp; vexpr ]) $$$ fresh () in
      add_node graph value;
      value
  | ListT typ ->
      let mixop = [ [ atom "ListT" ]; [] ] in
      let vtyp = in_typ graph typ in
      let value = CaseV (mixop, [ vtyp ]) $$$ fresh () in
      add_node graph value;
      value
  | TupleT typs ->
      let mixop = [ [ atom "TupleT" ]; [] ] in
      let vtyps = in_typs graph typs in
      let value = CaseV (mixop, [ vtyps ]) $$$ fresh () in
      add_node graph value;
      value
  | NameT var ->
      let mixop = [ [ atom "NameT" ]; [] ] in
      let vvar = in_var graph var in
      let value = CaseV (mixop, [ vvar ]) $$$ fresh () in
      add_node graph value;
      value
  | SpecT (var, targs) ->
      ...
      let value = CaseV (mixop, [ vvar; vtargs ]) $$$ fresh () in
      add_node graph value;
      value
  | AnyT ->
...
and in_tparam (graph : graph) (tparam : P4.tparam) : value =
  let value = TextV tparam.it $$$ fresh () in
  add_node graph value;
  value
...
and in_param (graph : graph) (param : P4.param) : value =
  ...
  let value = CaseV (mixop, [ vid; vdir; vtyp; vexpr_opt ]) $$$ fresh () in
  add_node graph value;
  value
...
and in_arg (graph : graph) (arg : P4.arg) : value =
  ...
  | NameA (id, expr_opt) ->
      ...
      let vid = in_id graph id in
      let vexpr_opt = in_opt in_expr graph expr_opt in
      let value = CaseV (mixop, [ vid; vexpr_opt ]) $$$ fresh () in
      add_node graph value;
      value
  | AnyA ->
...
and in_expr (graph : graph) (expr : P4.expr) : value =
  ...
  | BoolE { boolean } ->
      ...
      let value = CaseV (mixop, [ vboolean ]) $$$ fresh () in
      add_node graph value;
      value
  | StrE { text } ->
      let mixop = [ [ atom "StrE" ]; [] ] in
      let vtext = in_text graph text in
      let value = CaseV (mixop, [ vtext ]) $$$ fresh () in
      add_node graph value;
      value
  | NumE { num } ->
      let mixop = [ [ atom "NumE" ]; [] ] in
      let vnum = in_num graph num in
      let value = CaseV (mixop, [ vnum ]) $$$ fresh () in
      add_node graph value;
      value
  | VarE { var } ->
  ...
  | SeqE { exprs } ->
      ...
      let value = CaseV (mixop, [ vexprs ]) $$$ fresh () in
      add_node graph value;
      value
  | SeqDefaultE { exprs } ->
  ...
  | RecordE { fields } ->
      ...
      let value = CaseV (mixop, [ vfields ]) $$$ fresh () in
      add_node graph value;
      value
  ...
  | UnE { unop; expr } ->
      ...
      let vexpr = in_expr graph expr in
      let value = CaseV (mixop, [ vunop; vexpr ]) $$$ fresh () in
      add_node graph value;
      value
  | BinE { binop; expr_l; expr_r } ->
      ...
      let value = CaseV (mixop, [ vbinop; vexpr_l; vexpr_r ]) $$$ fresh () in
      add_node graph value;
      value
  | TernE { expr_cond; expr_then; expr_else } ->
      ...
      let value =
        CaseV (mixop, [ vexpr_cond; vexpr_then; vexpr_else ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | CastE { typ; expr } ->
  ...
  | MaskE { expr_base; expr_mask } ->
      ...
      let value = CaseV (mixop, [ vexpr_base; vexpr_mask ]) $$$ fresh () in
      add_node graph value;
      value
  | RangeE { expr_lb; expr_ub } ->
      ...
      let value = CaseV (mixop, [ vexpr_lb; vexpr_ub ]) $$$ fresh () in
      add_node graph value;
      value
  | SelectE { exprs_select; cases } ->
      ...
      let value = CaseV (mixop, [ vexprs_select; vcases ]) $$$ fresh () in
      add_node graph value;
      value
  | ArrAccE { expr_base; expr_idx } ->
      ...
      let value = CaseV (mixop, [ vexpr_base; vexpr_idx ]) $$$ fresh () in
      add_node graph value;
      value
  | BitAccE { expr_base; expr_lo; expr_hi } ->
      ...
      let value =
        CaseV (mixop, [ vexpr_base; vexpr_lo; vexpr_hi ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | ErrAccE { member } ->
      ...
      let value = CaseV (mixop, [ vmember ]) $$$ fresh () in
      add_node graph value;
      value
  | TypeAccE { var_base; member } ->
      ...
      let vmember = in_member graph member in
      let value = CaseV (mixop, [ vvar_base; vmember ]) $$$ fresh () in
      add_node graph value;
      value
  | ExprAccE { expr_base; member } ->
      ...
      let vexpr_base = in_expr graph expr_base in
      let vmember = in_member graph member in
      let value = CaseV (mixop, [ vexpr_base; vmember ]) $$$ fresh () in
      add_node graph value;
      value
  | CallFuncE { var_func; targs; args } ->
      ...
      let value = CaseV (mixop, [ vvar_func; vtargs; vargs ]) $$$ fresh () in
      add_node graph value;
      value
  | CallMethodE { expr_base; member; targs; args } ->
      ...
      let vargs = in_args graph args in
      let value =
        CaseV (mixop, [ vexpr_base; vmember; vtargs; vargs ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | CallTypeE { var_typ; member; targs; args } ->
      ...
      let vargs = in_args graph args in
      let value =
        CaseV (mixop, [ vvar_typ; vmember; vtargs; vargs ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | InstE { var_inst; targs; args } ->
      ...
      let vtargs = in_targs graph targs in
      let vargs = in_args graph args in
      let value = CaseV (mixop, [ vvar_inst; vtargs; vargs ]) $$$ fresh () in
      add_node graph value;
      value
...
and in_select_case (graph : graph) (select_case : P4.select_case) : value =
  ...
  let value = CaseV (mixop, [ vkeysets; vstate_label ]) $$$ fresh () in
  add_node graph value;
  value
...
and in_stmt (graph : graph) (stmt : P4.stmt) : value =
  ...
  | AssignS { expr_l; expr_r } ->
      ...
      let vexpr_l = in_expr graph expr_l in
      let vexpr_r = in_expr graph expr_r in
      let value = CaseV (mixop, [ vexpr_l; vexpr_r ]) $$$ fresh () in
      add_node graph value;
      value
  | SwitchS { expr_switch; cases } ->
      ...
      let value = CaseV (mixop, [ vexpr_switch; vcases ]) $$$ fresh () in
      add_node graph value;
      value
  | IfS { expr_cond; stmt_then; stmt_else } ->
      ...
      let value =
        CaseV (mixop, [ vexpr_cond; vstmt_then; vstmt_else ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | BlockS { block } ->
      ...
      let value = CaseV (mixop, [ vblock ]) $$$ fresh () in
      add_node graph value;
      value
  | ExitS ->
  ...
  | RetS { expr_ret } ->
      ...
      let value = CaseV (mixop, [ vexpr_ret ]) $$$ fresh () in
      add_node graph value;
      value
  ...
  | TransS { expr_label } ->
      ...
      let value = CaseV (mixop, [ vexpr_label ]) $$$ fresh () in
      add_node graph value;
      value
  | DeclS { decl } ->
      let mixop = [ [ atom "DeclS" ]; [] ] in
      let vdecl = in_decl graph decl in
      let value = CaseV (mixop, [ vdecl ]) $$$ fresh () in
      add_node graph value;
      value
...
and in_block (graph : graph) (block : P4.block) : value =
  ...
  let value = CaseV (mixop, [ vstmts ]) $$$ fresh () in
  add_node graph value;
  value
...
and in_switch_case (graph : graph) (switch_case : P4.switch_case) : value =
  ...
  | MatchC (switch_label, block) ->
      ...
      let vblock = in_block graph block in
      let value = CaseV (mixop, [ vswitch_label; vblock ]) $$$ fresh () in
      add_node graph value;
      value
  | FallC switch_label ->
      let mixop = [ [ atom "FallC" ]; [] ] in
      let vswitch_label = in_switch_label graph switch_label in
      let value = CaseV (mixop, [ vswitch_label ]) $$$ fresh () in
      add_node graph value;
      value
...
and in_decl (graph : graph) (decl : P4.decl) : value =
  ...
  | ConstD { id; typ; value; _ } ->
      ...
      let value = CaseV (mixop, [ vid; vtyp; vvalue ]) $$$ fresh () in
      add_node graph value;
      value
  | VarD { id; typ; init; _ } ->
      ...
      let value = CaseV (mixop, [ vid; vtyp; vinit ]) $$$ fresh () in
      add_node graph value;
      value
  | ErrD { members } ->
      let mixop = [ [ atom "ErrD" ]; [] ] in
      let vmembers = in_members graph members in
      let value = CaseV (mixop, [ vmembers ]) $$$ fresh () in
      add_node graph value;
      value
  | MatchKindD { members } ->
  ...
  | InstD { id; var_inst; targs; args; init; _ } ->
      ...
      let value =
        CaseV (mixop, [ vid; vvar_inst; vtargs; vargs; vinit ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | StructD { id; tparams; fields; _ } ->
      ...
      in
      let value = CaseV (mixop, [ vid; vtparams; vfields ]) $$$ fresh () in
      add_node graph value;
      value
  ...
  | EnumD { id; members; _ } ->
      ...
      let vid = in_id graph id in
      let vmembers = in_members graph members in
      let value = CaseV (mixop, [ vid; vmembers ]) $$$ fresh () in
      add_node graph value;
      value
  | SEnumD { id; typ; fields; _ } ->
      ...
      let vtyp = in_typ graph typ in
      let vfields = in_list (in_pair in_member in_expr) graph fields in
      let value = CaseV (mixop, [ vid; vtyp; vfields ]) $$$ fresh () in
      add_node graph value;
      value
  | NewTypeD { id; typdef; _ } ->
      ...
      let value = CaseV (mixop, [ vid; vtypdef ]) $$$ fresh () in
      add_node graph value;
      value
  ...
  | ValueSetD { id; typ; size; _ } ->
      ...
      let value = CaseV (mixop, [ vid; vtyp; vsize ]) $$$ fresh () in
      add_node graph value;
      value
  | ParserTypeD { id; tparams; params; _ } ->
      ...
      let value = CaseV (mixop, [ vid; vtparams; vparams ]) $$$ fresh () in
      add_node graph value;
      value
  | ParserD { id; params; cparams; locals; states; _ } ->
      ...
      let value =
        CaseV (mixop, [ vid; vparams; vcparams; vlocals; vstates ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | TableD { id; table; _ } ->
      ...
      let value = CaseV (mixop, [ vid; vtable ]) $$$ fresh () in
      add_node graph value;
      value
  ...
  | ControlD { id; params; cparams; locals; body; _ } ->
      ...
      let value =
        CaseV (mixop, [ vid; vparams; vcparams; vlocals; vbody ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | ActionD { id; params; body; _ } ->
      ...
      let vparams = in_params graph params in
      let vbody = in_block graph body in
      let value = CaseV (mixop, [ vid; vparams; vbody ]) $$$ fresh () in
      add_node graph value;
      value
  | FuncD { id; typ_ret; tparams; params; body; _ } ->
      ...
      let vbody = in_block graph body in
      let value =
        CaseV (mixop, [ vid; vtyp_ret; vtparams; vparams; vbody ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | ExternFuncD { id; typ_ret; tparams; params; _ } ->
      ...
      let vparams = in_params graph params in
      let value =
        CaseV (mixop, [ vid; vtyp_ret; vtparams; vparams ]) $$$ fresh ()
      in
      add_node graph value;
      value
  | ExternObjectD { id; tparams; mthds; _ } ->
      ...
      let value = CaseV (mixop, [ vid; vtparams; vmthds ]) $$$ fresh () in
      add_node graph value;
      value
  | PackageTypeD { id; tparams; cparams; _ } ->
      ...
      let vtparams = in_tparams graph tparams in
      let vcparams = in_cparams graph cparams in
      let value = CaseV (mixop, [ vid; vtparams; vcparams ]) $$$ fresh () in
      add_node graph value;
      value
...
and in_parser_state (graph : graph) (parser_state : P4.parser_state) : value =
  ...
  let vstate_label = in_state_label graph state_label in
  let vblock = in_block graph block in
  let value = CaseV (mixop, [ vstate_label; vblock ]) $$$ fresh () in
  add_node graph value;
  value
...
and in_table_key (graph : graph) (table_key : P4.table_key) : value =
  ...
  let value = CaseV (mixop, [ vexpr; vmatch_kind ]) $$$ fresh () in
  add_node graph value;
  value
...
and in_table_action (graph : graph) (table_action : P4.table_action) : value =
  ...
  let vvar = in_var graph var in
  let vargs = in_args graph args in
  let value = CaseV (mixop, [ vvar; vargs ]) $$$ fresh () in
  add_node graph value;
  value
...
and in_table_entries (graph : graph) (table_entries : P4.table_entries) : value
  ...
  let value =
    CaseV (mixop, [ vtable_entries_const; vtable_entries ]) $$$ fresh ()
  in
  add_node graph value;
  value
...
and in_table_default (graph : graph) (table_default : P4.table_default) : value
  ...
  let vtable_action = in_table_action graph table_action in
  let value =
    CaseV (mixop, [ vtable_default_const; vtable_action ]) $$$ fresh ()
  in
  add_node graph value;
  value
...
and in_table_custom (graph : graph) (table_custom : P4.table_custom) : value =
  ...
  let vexpr = in_expr graph expr in
  let value =
    CaseV (mixop, [ vtable_custom_const; vmember; vexpr ]) $$$ fresh ()
  in
  add_node graph value;
  value
...
and in_mthd (graph : graph) (mthd : P4.mthd) : value =
  ...
  | ExternConsM { id; cparams; _ } ->
      ...
      let vid = in_id graph id in
      let vcparams = in_cparams graph cparams in
      let value = CaseV (mixop, [ vid; vcparams ]) $$$ fresh () in
      add_node graph value;
      value
