        ...
        (String.concat ""
           (List.map2
              (fun vid atoms ->
                Format.asprintf "%s%s"
                  (Il.Print.string_of_vid vid)
                  (Il.Print.string_of_atoms atoms))
              vids mixop_t))
  | TupleN vids ->
      Format.asprintf "(%s)"
        (String.concat ", " (vids |> List.map Il.Print.string_of_vid))
  | OptN (Some vid) -> Format.asprintf "Some(%s)" (Il.Print.string_of_vid vid)
  | OptN None -> "None"
  | ListN [] -> "[]"
  | ListN vids ->
      Format.asprintf "[ %s ]"
        (String.concat ", " (List.map Il.Print.string_of_vid vids))
  | FuncN id -> Il.Print.string_of_defid id
