    ...
    (let open Command.Let_syntax in
     ...
     fun () ->
       try
         ...
         let _ =
           Interpret.Interp.run_typing ~debug ~profile ~derive:false spec_il
             program_p4
         in
         ()
       with Error (at, msg) -> Format.printf "%s\n" (string_of_error at msg))
...
let run_testgen_command =
  Command.basic
    ...
    (let open Command.Let_syntax in
     ...
     and filename_prem =
       flag "-prem" (required string)
         ~doc:"output file for premise classification"
     and dirname_derive =
     ...
     fun () ->
       try
         ...
         in
         let spec_il = Elaborate.Elab.elab_spec spec in
         Testgen.Classify.classify_spec ~filename:filename_prem spec_il;
         let program_p4 = P4frontend.Parse.parse_file includes_p4 filename_p4 in
         Testgen.Gen.gen ~dirname:dirname_derive spec_il program_p4;
         ()
       with Error (at, msg) -> Format.printf "%s\n" (string_of_error at msg))
